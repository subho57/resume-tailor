...
This is a **personal skill** installed at `~/.copilot/skills/jd-tailored-resume/` by
...
working (create it if needed). The CLI writes both `<FileName>.docx` and `<FileName>.pdf` there; keep the
...
```bash
python ~/.copilot/skills/jd-tailored-resume/scripts/check_keywords.py \
...
This skill directory (`~/.copilot/skills/jd-tailored-resume/`) contains:
